...
namespace Ssm.Engine.ScriptStatements {
    ...
    /// 定义一个变量
    /// </summary>
    public class Variable : ISemanticStatement {
        ...
        /// 获取类型
        /// </summary>
        public ScriptSemanticTypes Type { get { return ScriptSemanticTypes.Variable; } }
        ...
        public ScriptSegment Resolve(ScriptEngine engine, ScriptSegment segment, int line, string statement) {
            ...
            if (strs.Length > 1) throw new SirException(line, 0, "该语句只允许定义一个变量");
            string name = strs[0];
            // 判断变量是否合法
